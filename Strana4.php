...
?> 
  ...
  <script>
    function myMove() {
      let id = null; // Proměnná pro identifikaci intervalu animace
      const elem = document.getElementById("animate"); // Element, který se bude pohybovat
      let pos = 0; // Počáteční pozice tělesa
      clearInterval(id); // Zastavení předchozí animace
      id = setInterval(frame, 5); // Spuštění intervalu animace s funkcí frame, volanou každých 5 milisekund
      function frame() {
        if (pos == 75) {
          clearInterval(id);
        } else {
          pos++;
          elem.style.top = pos + "px"; // Nastavení nové vertikální pozice tělesa
          elem.style.left = pos + "px"; // Nastavení nové horizontální pozice tělesa
        }
      }
    }
  </script>
...
?>  
  ...
  <?php
// Definování proměnných a nastavení na prázdné hodnoty
$name = $email = $gender = $comment = $website = "";
// Kontrola, zda byl formulář odeslán pomocí metody POST
...
// Funkce pro úpravu vstupních dat
...
.custom-radio-label {
  ...
  <style>
    input[type="submit"] {
      color: white; /* Barva */
      border: 2px white; /* Barva rámečku */
      padding: 10px 20px; /* Odsazení */
      background-color: DarkGoldenRod; /* Barva pozadí rámečku */
    }
  </style>
<br>
<hr>
